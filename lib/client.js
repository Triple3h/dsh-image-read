(() => {
  window.__ModuleLoader__.load({
    ...
    factory: (require2) => {
      ...
      const { jsx, jsxs, Fragment } = require2("react/jsx-runtime");
...
.dshca-chat-usertext{flex:0 0 100%;white-space:pre-wrap;word-break:break-word;color:inherit;font:inherit}`;
      ...
      const attListeners = /* @__PURE__ */ new Set();
      let attRevision = 0;
      function attChanged() {
        attRevision += 1;
      ...
      function useAttRevision() {
        return react.useSyncExternalStore((l) => {
          attListeners.add(l);
          return () => attListeners.delete(l);
        }, () => attRevision, () => attRevision);
      }
      function attId() {
      ...
      function validateItems(items) {
        ...
        if (items.length > 1e4) throw new Error("Selection exceeds 10,000 files");
        let total = 0;
      ...
      async function walkEntry(entry, prefix = "") {
        ...
        if (entry.isFile) {
          ...
          return [{ file, path: normalizeRelativePath(rel, file.name) }];
        }
      ...
      async function upload(record, signal) {
        if (record.committed !== void 0) return record.modelText;
        if (record.inflight !== void 0) return record.inflight;
        const task = (async () => {
          ...
          try {
            ...
            if (batchId !== void 0) fetch(`${ATT_API}/batches/${encodeURIComponent(batchId)}`, { method: "DELETE" }).catch(() => {
            });
            record.status = "error";
      ...
      function showPasteNotice(onConfirm, onCancel) {
        ...
        copy.textContent = "\u4F60\u7C98\u8D34\u4E86\u56FE\u7247\u6216\u6587\u4EF6\u3002DSH Image Read \u4F1A\u628A\u5B83\u4EEC\u590D\u5236\u5230\u5F53\u524D\u4F1A\u8BDD\u5DE5\u4F5C\u533A\u7684\u4E34\u65F6\u9644\u4EF6\u76EE\u5F55\uFF08.dsh/tmp/attachments/\uFF09\uFF0C\u5E76\u5728\u53D1\u9001\u65F6\u968F\u6D88\u606F\u4E00\u8D77\u4EA4\u7ED9\u6A21\u578B\u3002";
        ...
        cancelBtn.className = "dshca-notice-cancel";
        cancelBtn.textContent = "\u53D6\u6D88";
        const okBtn = document.createElement("button");
        okBtn.type = "button";
        okBtn.className = "dshca-notice-ok";
        ...
        okBtn.addEventListener("click", () => {
          try {
            if (cb.checked) localStorage.setItem(NOTICE_KEY, "1");
          } catch {
          }
          noticeDismissed = noticeDismissed || cb.checked;
          overlay.remove();
          onConfirm();
        });
      ...
      function AttachButton(props) {
        ...
        const accept = react.useCallback(async (itemsOrPromise) => {
          ...
          try {
            ...
            setMessage(c instanceof Error ? c.message : String(c));
          } finally {
            setBusy(false);
          }
        }, [props.add]);
        return h(
          ...
          open && h(
            "div",
            { className: "dshca-menu", role: "menu" },
      ...
      function AttachmentChips(props, className) {
        ...
        return h("div", { className }, ...occurrences.map((occ) => {
          ...
          const meta = status === "uploading" ? `${record.uploaded}/${record.items.length}` : status === "uploaded" ? "copied" : record === void 0 ? "unavailable" : humanBytes(record.total);
          return h(
            "div",
            { className: "dshca-chip", "data-status": status, key: occ.occurrenceId },
            ...
            h("button", { type: "button", className: "dshca-remove", "aria-label": `Remove ${record?.label ?? occ.label}`, disabled: props.input.phase !== "plain", onClick: () => props.remove(occ) }, "\xD7")
          );
        }));
      }
      ...
      function attachmentCopy() {
        const zh2 = typeof navigator !== "undefined" && navigator.language.toLowerCase().startsWith("zh");
        return zh2 ? {
          nav: "\u9644\u4EF6\u7BA1\u7406",
          title: "\u9644\u4EF6\u7BA1\u7406\u4E0E\u7A7A\u95F4\u6E05\u7406",
          ...
          refresh: "\u5237\u65B0\u7EDF\u8BA1",
          currentScope: "\u5F53\u524D\u4F1A\u8BDD",
          workspaceScope: (u) => `\u5F53\u524D\u5DE5\u4F5C\u533A \xB7 ${u.sessionDirectories} \u4E2A\u6709\u9644\u4EF6\u7684\u4F1A\u8BDD`,
          cleanCurrent: "\u6E05\u7406\u5F53\u524D\u4F1A\u8BDD\u9644\u4EF6",
          cleanWorkspace: "\u6E05\u7406\u5F53\u524D\u5DE5\u4F5C\u533A\u5168\u90E8\u4F1A\u8BDD\u9644\u4EF6",
          confirmCurrent: "\u5C06\u53EA\u5220\u9664\u5F53\u524D\u4F1A\u8BDD\u4E2D\u7531\u672C\u63D2\u4EF6\u521B\u5EFA\u7684\u4E34\u65F6\u9644\u4EF6\u3002\u8BF7\u518D\u6B21\u70B9\u51FB\u786E\u8BA4\u3002",
          confirmWorkspace: "\u5C06\u5220\u9664\u5F53\u524D\u5DE5\u4F5C\u533A\u6240\u6709\u4F1A\u8BDD\u7531\u672C\u63D2\u4EF6\u521B\u5EFA\u7684\u4E34\u65F6\u9644\u4EF6\uFF1B\u4E0D\u4F1A\u5F71\u54CD\u5176\u4ED6\u5DE5\u4F5C\u533A\u3002\u8BF7\u518D\u6B21\u70B9\u51FB\u786E\u8BA4\u3002",
          confirmCurrentButton: "\u518D\u6B21\u70B9\u51FB\uFF1A\u6E05\u7406\u5F53\u524D\u4F1A\u8BDD",
          confirmWorkspaceButton: "\u518D\u6B21\u70B9\u51FB\uFF1A\u6E05\u7406\u5F53\u524D\u5DE5\u4F5C\u533A",
          cancel: "\u53D6\u6D88",
          noSession: "\u8BF7\u5148\u6253\u5F00\u4E00\u4E2A\u4F1A\u8BDD\u3002",
          loading: "\u6B63\u5728\u8BFB\u53D6\u5F53\u524D\u4F1A\u8BDD\u9644\u4EF6\u2026",
          cleanedCurrent: (r) => `\u5DF2\u6E05\u7406\u5F53\u524D\u4F1A\u8BDD ${r.deletedFiles} \u4E2A\u6587\u4EF6\uFF08${humanBytes(r.deletedBytes)}\uFF09\u3002`,
          cleanedWorkspace: (r) => `\u5DF2\u6E05\u7406\u5F53\u524D\u5DE5\u4F5C\u533A ${r.deletedSessionDirectories} \u4E2A\u4F1A\u8BDD\u76EE\u5F55\u3001${r.deletedFiles} \u4E2A\u6587\u4EF6\uFF08${humanBytes(r.deletedBytes)}\uFF09\u3002`
        } : {
          nav: "Attachments",
          title: "Attachment management & cleanup",
          ...
          workspaceScope: (u) => `Active workspace \xB7 ${u.sessionDirectories} sessions with attachments`,
          cleanCurrent: "Clean active session",
          cleanWorkspace: "Clean every session in this workspace",
          confirmCurrent: "Only temporary attachments created by this plugin in the active session will be deleted. Click again to confirm.",
          confirmWorkspace: "Temporary attachments in every session in this workspace will be deleted. Other workspaces are not affected. Click again to confirm.",
          ...
          cleanedCurrent: (r) => `Removed ${r.deletedFiles} files from the active session (${humanBytes(r.deletedBytes)}).`,
          cleanedWorkspace: (r) => `Removed ${r.deletedFiles} files from ${r.deletedSessionDirectories} session directories in this workspace (${humanBytes(r.deletedBytes)}).`
        };
      }
      ...
      function AttachmentSettings(props) {
        ...
        const [status, setStatus] = react.useState(sessionId === void 0 ? copy.noSession : copy.loading);
        const [busy, setBusy] = react.useState(false);
        const [confirming, setConfirming] = react.useState(null);
        const load = react.useCallback(async (signal) => {
          if (sessionId === void 0) {
            setUsage({ sends: 0, files: 0, bytes: 0 });
            setWsUsage({ sessionDirectories: 0, sends: 0, files: 0, bytes: 0 });
          ...
          try {
            ...
            if (c?.name !== "AbortError") setStatus(c instanceof Error ? c.message : String(c));
          } finally {
        ...
        const clean = async (scope) => {
          ...
          try {
            ...
            if (scope === "workspace") {
              setUsage({ sends: 0, files: 0, bytes: 0 });
              setWsUsage({ sessionDirectories: 0, sends: 0, files: 0, bytes: 0 });
              setStatus(copy.cleanedWorkspace(r));
            } else {
              setUsage({ sends: 0, files: 0, bytes: 0 });
              setWsUsage((cur) => ({ sessionDirectories: Math.max(0, cur.sessionDirectories - 1), sends: Math.max(0, cur.sends - r.deletedSends), files: Math.max(0, cur.files - r.deletedFiles), bytes: Math.max(0, cur.bytes - r.deletedBytes) }));
              setStatus(copy.cleanedCurrent(r));
            }
          } catch (c) {
            setStatus(c instanceof Error ? c.message : String(c));
          } finally {
            setBusy(false);
          }
        };
        return h(
          ...
          h(
            "div",
            { className: "dshca-settings-actions" },
            h("button", { type: "button", className: "dshca-settings-action", disabled: busy || sessionId === void 0, onClick: () => void load() }, copy.refresh),
            h("button", { type: "button", className: "dshca-settings-action", "data-danger": true, disabled: busy || sessionId === void 0 || usage.sends === 0, onClick: () => void clean("session") }, confirming === "session" ? copy.confirmCurrentButton : copy.cleanCurrent),
            h("button", { type: "button", className: "dshca-settings-action", "data-danger": true, disabled: busy || sessionId === void 0 || wsUsage.sends === 0, onClick: () => void clean("workspace") }, confirming === "workspace" ? copy.confirmWorkspaceButton : copy.cleanWorkspace),
            confirming !== null && h("button", { type: "button", className: "dshca-settings-action", disabled: busy, onClick: () => {
              setConfirming(null);
              setStatus("");
            } }, copy.cancel)
          ),
          h("div", { className: "dshca-settings-status" }, copy.workspaceScope(wsUsage), " \xB7 ", humanBytes(wsUsage.bytes)),
      ...
      function foldAttachmentDiv(div) {
        ...
        if (blocks.length === 0) {
          if (!warned.has(div)) {
            warned.add(div);
            console.warn("dsh-image-read: fold skipped (parse failed):", text.slice(0, 300));
          }
          return;
        }
        if (div.dataset.pasteFolded === void 0) div.dataset.pasteFolded = "1";
        if (div.dataset.pasteId === void 0) div.dataset.pasteId = attId();
        ...
        for (const block of blocks) {
          ...
          for (const file of block.files) {
            ...
            if (block.raw !== void 0) {
              ...
              if (file.original !== void 0 && file.original !== file.path) tipLines.push("\u539F\u59CB\uFF1A" + file.original);
            ...
            chip.addEventListener("click", () => {
              navigator.clipboard?.writeText(absolute).then(() => showToast("\u5DF2\u590D\u5236\u8DEF\u5F84\uFF1A" + absolute), () => showToast("\u590D\u5236\u5931\u8D25\uFF1A" + absolute));
            });
      ...
      function foldScan() {
        ...
        while ((node = walker.nextNode()) !== null) {
          ...
          seen.add(container);
          if (container.dataset.pasteFolded !== void 0) {
            try {
              foldAttachmentDiv(container);
            } catch {
            }
            continue;
          }
          try {
            ...
            console.warn("dsh-image-read fold failed:", c);
          }
        }
      }
      const en = {
      ...
      function apply(ctx) {
        ...
        try {
          ...
          console.warn("dsh-image-read: observer setup failed:", c);
        }
        ...
        try {
          ...
          console.warn("dsh-image-read: initial fold failed:", c);
        }
        ctx.effect(() => () => {
          chatObserver.disconnect();
          clearInterval(sweep);
          if (foldTimer !== null) clearTimeout(foldTimer);
        }, "image-read: bubble fold observer");
        const source = {
          trigger: "@",
          name: ATT_SOURCE,
          order: 1e3,
          ...
          codec: {
            ...
            serialize: (ref, signal) => {
              ...
              if (rec === void 0) return Promise.reject(new Error("Attachment selection is no longer available in this browser tab"));
              return upload(rec, signal);
            }
          }
        };
        ...
        const onPaste = (event) => {
          ...
          const proceed = () => {
            ...
            let items;
            try {
              items = filesFromList(files);
            } catch (c) {
              showToast(c instanceof Error ? c.message : String(c));
              return;
            }
        ...
        }, "image-read: page drop listener");
        const inputFor = (sessionId) => {
          const actx = sessions.scope(sessionId);
          if (actx === void 0) throw new Error(`Attachment session is not active: ${sessionId}`);
          ...
          for (const item of items) {
            ...
            if (typeof input.state.subscribe === "function") {
              const unsubscribe = input.state.subscribe(() => {
                ...
                if (alive || record.inflight !== void 0) return;
                unsubscribe();
                records.delete(ref);
                attChanged();
              });
        ...
        ctx.inject(["slots", "conversation", "sessions", "inputTriggers"], (scope2) => {
          scope2.slots.inject("conversation.input.left", () => scope2.slots.register({ name: "conversation.input.left", id: "dsh-image-read-attach-btn", order: -100, inject: (sessionId) => ({ add: (items) => attAdd(sessionId, items) }) }, AttachButton));
          scope2.slots.inject("conversation.input.dock", () => scope2.slots.register({ name: "conversation.input.dock", id: "dsh-image-read-dock", order: 5, inject: (sessionId) => ({ remove: (occ) => attRemove(sessionId, occ) }) }, AttachmentDock));
          scope2.slots.inject("settings.section", () => scope2.slots.register({ name: "settings.section", id: "attachments", order: 20, label: () => attachmentCopy().nav, inject: () => ({ sessions }) }, AttachmentSettings));
        });
