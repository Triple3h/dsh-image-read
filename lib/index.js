...
const JSON_TEMPLATE_INSTRUCTION =
  'Respond with ONE JSON object only, no markdown fences, no commentary. ' +
  'Fill this exact structure with your findings from the image ' +
  '(do not repeat this template literally, replace every value):\n' +
  JSON.stringify(
    {
      summary: 'one paragraph describing the image',
      ocr: { full_text: 'all visible text concatenated', lines: [{ text: 'one line', language: 'en' }] },
      layout: { regions: [{ type: 'title | heading | paragraph | list | table | chart | form | code | image', reading_order: 1, text: 'region text' }] },
      uncertainty: ['anything unreadable or ambiguous'],
    },
    null, 0,
  );

function buildVisionPrompt(focus, mode = 'general', imageCount = 1) {
  const baseRules =
    'You are a vision parsing engine for a text-only LLM.\n' +
    'Convert everything in the image into structured evidence.\n\n' +
    'Rules:\n' +
    '1. Cover all visible text, structure, and layout as thoroughly as possible.\n' +
  ...
  if (mode === 'ocr') {
    return baseRules + '\n\nMode: OCR. Extract ALL visible text, preserving original layout (headings, lists, tables). Put the complete text in ocr.full_text and break it into ocr.lines.';
  }
  if (imageCount > 1) {
    return baseRules + `\n\nYou have ${imageCount} images. For EACH image, produce the same structured format. Number the layout regions continuously across all images.`;
  }
  return baseRules + '\n\nProvide a comprehensive, detailed description of everything visible.';
}
...
function validateAndNormalize(result) {
  ...
  const missing = schemaViolations(VISION_RESULT_SCHEMA, normalized);
  if (missing.length > 0) throw new Error(`Vision result missing or invalid fields: ${missing.join(', ')}`);
  return normalized;
}
...
export async function readImage(input, signal, cfg) {
  const { image_path, compare_with, focus = '', mode = 'general' } = input;
  const tmpFiles = [];
  ...
  try {
    ...
    const prompt = buildVisionPrompt(focus, mode, imageCount);
    ...
    for (const provider of providers) {
      ...
      try {
        ...
        try { parsed = JSON.parse(text); } catch {
          ...
          else throw new Error('API returned non-JSON output');
        }
...
function normalizeConfig(raw = {}) {
  ...
  if (Array.isArray(raw.providers) && raw.providers.length > 0) {
    for (const p of raw.providers) {
      ...
      providers.push({ name: p.name || 'unnamed', baseUrl: p.baseUrl.replace(/\/$/, ''), apiKey: p.apiKey, model: p.model });
    }
  }
  if (providers.length === 0 && raw.baseUrl && raw.apiKey) {
    providers.push({ name: 'primary', baseUrl: raw.baseUrl.replace(/\/$/, ''), apiKey: raw.apiKey, model: raw.model || DEFAULT_MODEL });
  }
  if (providers.length === 0) {
    providers.push({ name: 'default', baseUrl: DEFAULT_API_URL.replace('/chat/completions', ''), apiKey: process.env.MIMO_API_KEY || '', model: DEFAULT_MODEL });
  }
  return {
    providers,
...
function makeTool(cfg) {
  return {
    ...
    async execute(args, exec) {
      ...
      return await readImage({ image_path: imagePath, compare_with: args.compare_with, focus: args.focus, detail_level: args.detail_level, mode: args.mode }, exec.signal, cfg);
    },
...
export function apply(ctx, config = {}) {
  ...
  const resolveApiKey = () => {
    try {
      ...
      if (credentials?.resolve) {
        ...
        const resolved = credentials.resolve(ref);
  ...
  const buildToolConfig = () => {
    ...
    if (Array.isArray(settings.providers) && settings.providers.length > 0) {
      settings.providers = settings.providers.map((p, i) => ({ ...p, apiKey: i === 0 ? resolveApiKey() : p.apiKey }));
    }
  ...
  installSettingsSection(ctx, IMAGE_READ_NS, Config, config, {
    ...
    onChange: () => { ctx.tools.register(makeToolConfig(buildToolConfig())); },
  });

  const cfg = buildToolConfig();
  if (cfg.providers.some((p) => !p.apiKey) && typeof ctx.logger?.warn === 'function') {
    ctx.logger.warn('[image-read] No API key configured (set providers[].apiKey, IMAGE_READ_*_API_KEY env var, or MIMO_API_KEY env var). read_image_mimo will fail on use.');
  }
  ctx.tools.register(makeTool(cfg));
}
